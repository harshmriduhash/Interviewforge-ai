        ...
        <div className="min-h-screen bg-background flex flex-col items-center justify-center p-4 relative overflow-hidden">
            ...
            <div className="max-w-2xl w-full z-10 flex flex-col gap-8">
                ...
                <div className="flex justify-between items-center px-2">
                    ...
                    <div className="absolute top-[48px] left-0 w-full h-px bg-border -z-10" />
                </div>
...
function Step1({ next }: { next: () => void }) {
    return (
        <div className="text-center space-y-8 flex flex-col items-center justify-center h-full pt-10">
            ...
            <button
                onClick={next}
                className="px-10 py-4 bg-primary text-white text-lg font-bold rounded-xl hover:bg-primary-hover shadow-lg shadow-primary/20 transition-all mt-8"
            >
        ...
        <div className="space-y-8">
            ...
            <div className="space-y-6">
                <div className="space-y-3">
                    ...
                    <select
                        value={data.role}
                        onChange={(e) => set({ ...data, role: e.target.value })}
                ...
                <div className="space-y-3">
                    ...
                    <div className="flex justify-between gap-2">
                        {levels.map(l => (
                            <button
                                key={l}
                                onClick={() => set({ ...data, experience: l })}
                                className={cn(
                                    "flex-1 py-3 rounded-lg border font-bold transition-all",
                                    data.experience === l ? "border-primary bg-primary/10 text-primary shadow-[0_0_10px_rgba(255,92,0,0.1)]" : "border-border text-text-muted hover:border-text-muted"
                                )}
                            >
                                {l}
                            </button>
    ...
    return (
        <div className="space-y-8">
            ...
            <div className="flex items-center gap-4 pt-4">
                <button
                    onClick={next}
                    className="flex-1 py-4 bg-primary text-white text-lg font-bold rounded-xl hover:bg-primary-hover shadow-lg shadow-primary/20 transition-all"
                >
...
function Step4({ data, set, next }: any) {
    ...
    return (
        <div className="space-y-8">
            ...
            <button
                onClick={next}
                className="w-full py-4 bg-primary text-white text-lg font-bold rounded-xl hover:bg-primary-hover shadow-lg shadow-primary/20 transition-all"
            >
                Complete Profiles →
            </button>
        </div>
    ...
    return (
        <div className="text-center space-y-8 flex flex-col items-center justify-center h-full pt-10">
            ...
            <div className="flex flex-col sm:flex-row gap-4 w-full">
                <Link
                    href="/session/demo"
                    className="flex-1 py-4 bg-primary text-white text-lg font-bold rounded-xl hover:bg-primary-hover shadow-lg shadow-primary/20 transition-all text-center"
                >
                    Start My First Session →
                </Link>
                <Link
                    href="/dashboard"
                    className="flex-1 py-4 border border-border hover:border-primary text-white text-lg font-bold rounded-xl transition-all text-center"
                >
                    Explore Dashboard
                </Link>
            </div>
