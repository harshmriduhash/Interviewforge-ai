...
export default function LoginPage() {
    ...
    return (
        <AuthLayout title="Welcome back" subtitle="Ready for your next practice session?">
            <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
                ...
                <form onSubmit={handleSubmit(onSubmit)} style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                    ...
                    <div>
                        ...
                        <input {...register("password")} type="password" placeholder="Enter your password" style={inputStyle}
                            onFocus={(e) => (e.currentTarget.style.borderColor = ORANGE)}
                            onBlur={(e) => (e.currentTarget.style.borderColor = BORDER)} />
                        {errors.password && <p style={{ color: ERROR_COLOR, fontSize: 12, marginTop: 4 }}>{errors.password.message}</p>}
                    </div>

                    <button disabled={isSubmitting} style={{ width: "100%", padding: "16px", borderRadius: 12, background: ORANGE, color: "#fff", fontWeight: 700, fontSize: 16, border: "none", cursor: isSubmitting ? "not-allowed" : "pointer", opacity: isSubmitting ? 0.6 : 1, boxShadow: "0 4px 20px rgba(255,92,0,0.3)", transition: "background 0.2s, transform 0.1s" }}
                ...
                <p style={{ textAlign: "center", fontSize: 14, color: MUTED }}>
                    ...
                    <Link href="/auth/signup" style={{ color: ORANGE, fontWeight: 700, textDecoration: "none" }}>Get started for free</Link>
                </p>
            </div>
        </AuthLayout>
    );
